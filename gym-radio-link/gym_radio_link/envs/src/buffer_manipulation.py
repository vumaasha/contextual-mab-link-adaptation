...
def extract_next_bits_with_zero_padding(buffer_, index, nrof_bits):
    ...
    if (index + nrof_bits) <= buffer_size:
        ...
        extracted_bits = itpp.concat(extracted_bits, itpp.zeros_b(nrof_padding_bits))
...
def extract_next_bits_with_wraparound(buffer_, index, nrof_bits):
    ...
    if (index + nrof_bits) < buffer_size:
        ...
        extracted_bits = itpp.concat(extracted_bits, buffer_.right(buffer_size - index))
        ...
        while (extracted_bits.length() < nrof_bits):
            ...
            if (nrof_remaining_bits > buffer_size):
                extracted_bits = itpp.concat(extracted_bits, buffer_)
            else:
                extracted_bits = itpp.concat(extracted_bits, buffer_.left(nrof_remaining_bits))
...
def add_values_with_wraparound(buffer_, index, values):
    buffer_size = buffer_.length()
    temp_buffer = itpp.zeros(buffer_size)
